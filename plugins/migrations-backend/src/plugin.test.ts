...
import { migrationsPlugin } from './plugin.ts';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [migrationsPlugin],
    });

    await request(server).get('/api/migrations/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/migrations/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/migrations/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/migrations/todos/${createdTodoItem.id}`)
